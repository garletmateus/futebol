...
function criarClienteMercadoPago() {
  ...
  if (!accessToken) {
    throw new Error(
      "Configure MERCADO_PAGO_ACCESS_TOKEN no arquivo .env"
    );
  }
...
function normalizarItens(itens) {
  return itens.map((item) => ({
    title: String(
      item.nome || "Camisa Resenha Sports"
    ).trim(),
  ...
  try {
    ...
    if (!pedidoId || !itens.length) {
      return res.status(400).json({
        erro: "Informe o pedido e os itens para criar o pagamento."
      });
    }
    ...
    const result = await preference.create({
      body: {
        ...
        notification_url:
          `${appUrl}/api/pagamentos/mercadopago/webhook`,
    ...
    await db.query(
      `UPDATE pedidos
       SET mercado_pago_preference_id = $1,
           status_pagamento = $2
       WHERE id = $3`,
      [
        result.id,
